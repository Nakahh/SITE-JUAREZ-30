...
export default async function PropertiesPage({
  ...
  if (minPreco) {
    where.price = { ...where.price, gte: Number.parseFloat(minPreco) }
  }
  if (maxPreco) {
    where.price = { ...where.price, lte: Number.parseFloat(maxPreco) }
  }
  if (quartos) {
    where.bedrooms = { gte: Number.parseInt(quartos) }
  }
  if (banheiros) {
    where.bathrooms = { gte: Number.parseInt(banheiros) }
  }
  if (garagem) {
    // Supondo que garagem é boolean no banco, aqui você pode filtrar por true/false
    // Se quiser quantidade mínima de vagas, ajuste o modelo e código
    where.garage = garagem === "0" ? false : true
  }
  if (minArea) {
